...
public class DaoImpl implements Dao
{
    ...
    public Book addBook(Book book) throws UserException {
    	String sql = "Insert into book(book_name,book_price,authorId) values (?,?,?)";
        try {
        	ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
            ps.setString(1, book.getBookName());
            ps.setBigDecimal(2, BigDecimal.valueOf(book.getBookPrice()));
    ...
    public List<Book> listAllBooks() {
        ...
        try {
            ...
            while (this.rs.next()) {
                ...
                book.setAuthorId(BigInteger.valueOf((rs.getLong("authorId"))));
                list.add(book);
            }
        }
        catch (SQLException e) {
    ...
    public List<Book> authorBooks(Book book) {
        ...
        String sql = "select book_id, book_name from book where authorId = ?";
        try {
            ps = connection.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                authorBook.setBookId((BigInteger.valueOf(rs.getInt("book_id"))));
                authorBook.setBookName(rs.getString("book_name"));
                authorList.add(authorBook);
            }
        }
        catch (SQLException e) {
	...
	public Author deleteAuthor(Author author) {
		String sql = "delete from author where author_id is ?";
        try {
            ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
        	ps.setInt(1, (author.getAuthorId().intValue()));            
            int noOfRec = ps.executeUpdate();
            BigInteger generatedId = BigInteger.valueOf(0L);
            rs = ps.getGeneratedKeys();
            if (rs.next()) {
